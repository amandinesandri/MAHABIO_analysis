#!/bin/bash
#SBATCH --job-name=align_full_array_on_mute
#SBATCH --output=logs/align_full_arrayon_mute_%A_%a.out
#SBATCH --error=logs/align_full_array_on_mute_%A_%a.err
#SBATCH --ntasks=1
...
CMD=minimap2 -ax map-ont "$TEMP_FASTA" "$long_reads" | samtools sort -@8 -o "${BAM_DIR}/${method}/${sample}/long/${BASENAME}_long.bam"
echo $CMD
CMD=samtools index "${BAM_DIR}/${method}/${sample}/long/${BASENAME}_long.bam"
echo $CMD

# Mapping short reads
echo "🔄 Indexation BWA"
CMD=bwa index "$TEMP_FASTA"
echo $CMD
echo "🔄 Mapping des short reads avec BWA MEM"
CMD=bwa mem -t 8 "$TEMP_FASTA" "$short_R1" "$short_R2" | samtools sort -@8 -o "${BAM_DIR}/${method}/${sample}/short/${BASENAME}_short.bam"
echo $CMD
CMD=samtools index "${BAM_DIR}/${method}/${sample}/short/${BASENAME}_short.bam"
echo $CMD
# Nettoyage
